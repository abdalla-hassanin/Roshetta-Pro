      ...
      },
      {
        ...
        'onTap': () {
          myDialogWarning(
              ...
              onYesTap: () {
                context.read<PharmacyAuthCubit>().pharmacySignOut();
              });
        }
      },
    ];
    return BlocBuilder<PharmacyAuthCubit, PharmacyAuthState>(
      builder: (context, state) {
        if (state is PharmacySignOutSuccess) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            Navigator.pushNamedAndRemoveUntil(
                context, Routes.coreScreen, (route) => false);
          });
        } else if (state is PharmacySignOutError) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
